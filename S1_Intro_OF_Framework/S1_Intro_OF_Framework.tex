...
More information can be found on the \href{https://www.openfoam.com/documentation/tutorial-guide/3-compressible-flow/3.1-steady-turbulent-flow-over-a-backward-facing-step}{OpenFOAM website}.
...
Change the density in function of the fluid. The keyword for \ofkeyword{executeControl} is included in order to compute total pressure in every time step, so that average values in the next
functions can be used. If this statement is not included, by default it is executed and written just in the \ofkeyword{writeTime}, i.e. in the times when fields are written fo disc.
The \\
...
statement. The functions \filename{system/vorticity}, 
...
Switch the displayed variable to $Q$ \includegraphics{Q_icon}. Note that the range is
...
\tip{For computation of turbulence variables, you can see this 
	\href{https://www.cfd-online.com/Wiki/Turbulence_free-stream_boundary_conditions}{web page in CFD online}. Get a relationship between $\varepsilon$ and $\omega$
