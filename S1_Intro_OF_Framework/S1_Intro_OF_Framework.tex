...
\begin{lstlisting}[style=terminal]
	...
	# Get the functions to compute voriticy and Q
	foamGet vorticity
	foamGet Q
\end{lstlisting} 
...
and the file \filename{system/inletTotalPressureAverage} is the same, but with the \texttt{name outlet;} statement. The functions \filename{system/vorticity} and \filename{system/Q} have to
kept unmodified.

Finally, in order the solver to read all this functions, they have to be included in the \filename{system/controlDict} file:
